...
#' 
mi_fam_k5 <- mi_fam %>%
  mutate(g=grupos_ward_k5) %>%
  group_by(g) %>%
  summarise_all(sum) %>%
  select(-g) %>% 
  data.frame
mi_fam_k5 %>% rowSums %>% sort
mi_fam_k5_chao <- estimacion_riqueza_chao(
  mc = mi_fam_k5,
  n_raras = 10)
mi_fam_k5_chao$asintoticos_estimacion
mi_fam_k5_chao$no_asintoticos_rarefaccion_extrapolacion
mi_fam_k5_chao$no_asintoticos_rarefaccion_extrapolacion_grafico