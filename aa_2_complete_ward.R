...
#' Para UPGMA:
#' 
anch_sil_complete <- calcular_anchuras_siluetas(
  mc_orig = mi_fam, 
...
#' 
u_dend_reord <- reorder.hclust(lista_cl$cl_complete, mi_fam_norm_d)
...
plot(cl_pvclust_complete, hang = -1)
# Añadir rectángulos a los grupos significativos
lines(cl_pvclust_complete)
pvrect(cl_pvclust_complete, alpha = 0.91, border = 4)
#' 
#' #### Ward
#' 
cl_pvclust_ward <-
...
#' 
#' UPGMA:
(grupos_complete_k2 <- as.factor(cutree(lista_cl$cl_complete, k = 2)))
#' 
...
#' 
saveRDS(grupos_complete_k2, 'grupos_complete_k2.RDS')
