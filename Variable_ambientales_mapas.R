...
mapa_cuadros_pendiente <- mapView(
  ...
  addStaticLabels(label = round(bci_env_grid$pendiente_media, 1), textsize = "15pt") %>%
  leaflet::setView(
    lng = -79.85136,
    lat = 9.15097,
    zoom = 17)
...
mapa_cuadros_nit <- mapView(
  ...
  col.regions = rojo,
  zcol = 'N') %>%
  addStaticLabels(label = round(bci_env_grid$N, 1), textsize = "15pt") %>%
  leaflet::setView(
    lng = -79.85136,
    lat = 9.15097,
    zoom = 17)
...
mapa_cuadros_ph <- mapView(
  ...
  zcol = 'pH') %>%
  addStaticLabels(label = round(bci_env_grid$pH, 1), textsize = "15pt") %>%
  leaflet::setView(
    lng = -79.85136,
    lat = 9.15097,
    zoom = 17)
